...
const TodoDetailModal: React.FC<{ todoId: string; onClose: () => void; onOpenCategory: (cat: Category) => void }> = ({ todoId, onClose, onOpenCategory }) => {
  ...
  const removeAssignee = (userId: string) => {
      const currentAssigneeIds = todo.assigneeIds || [];
      const newAssigneeIds = currentAssigneeIds.filter(id => id !== userId);
      updateTodo(todo.id, { assigneeIds: newAssigneeIds });
  };
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {todo.assignees && todo.assignees.length > 0 && (
              <div className="pt-4 border-t dark:border-gray-700">
                  <label className="block text-xs font-semibold text-gray-400 uppercase mb-2">Assignees</label>
                  <div className="flex flex-wrap gap-2">
                      {todo.assignees.map(assignee => (
                          <div key={assignee.id} className="flex items-center space-x-2 bg-gray-50 dark:bg-gray-700/50 px-2 py-1 rounded-full border border-gray-200 dark:border-gray-600 group">
                              ...
                              <span className="text-xs text-gray-700 dark:text-gray-300">{assignee.name || assignee.email}</span>
                              <button 
                                onClick={() => removeAssignee(assignee.id)}
                                className="ml-1 p-0.5 rounded-full text-gray-400 hover:text-red-500 hover:bg-gray-200 dark:hover:bg-gray-600 opacity-0 group-hover:opacity-100 transition-opacity"
